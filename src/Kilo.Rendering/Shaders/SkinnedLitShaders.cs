...
public static class SkinnedLitShaders
{
    public const string WGSL = """
        // Shared structs (same as BasicLit)
        struct CameraData {
            view: mat4x4<f32>,
        ...
        // Bindings - groups 0-3 same as BasicLit, group 4 for joints
        ...
        @group(4) @binding(0) var<uniform> joints: JointMatrices;
        ...
        fn vs_main(
            ...
            // Skinning: blend joint matrices
            let j0 = joint_indices.x;
            let j1 = joint_indices.y;
            let j2 = joint_indices.z;
            let j3 = joint_indices.w;

            let skin_matrix =
                joint_weights.x * joints.matrices[j0] +
                joint_weights.y * joints.matrices[j1] +
                joint_weights.z * joints.matrices[j2] +
                joint_weights.w * joints.matrices[j3];

            // Apply skinning then model transform
            let skinned_pos = skin_matrix * vec4<f32>(position, 1.0);
            ...
            out.normal = normalize((object.model * vec4<f32>(skinned_normal, 0.0)).xyz);
            out.uv = uv;
        ...
        // Fragment shader is identical to BasicLit
        @fragment
